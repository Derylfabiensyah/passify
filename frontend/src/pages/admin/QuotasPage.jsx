  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div>
          <span className="text-xs font-bold text-zinc-100">{day.dayLabel}</span>
          {isToday && (
            <span className="ml-1.5 text-[9px] text-emerald-400 font-bold bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/30">
              HARI INI
            </span>
          )}
        </div>
      ...
      {day.is_closed ? (
        <div className="text-center py-2.5">
          <Ban className="w-4 h-4 text-rose-400 mx-auto mb-1" />
        ...
        <>
          <div className="w-full h-1.5 bg-zinc-900 rounded-full overflow-hidden mb-2.5">
            <div
...
function TimeSlotCard({ slot, onEdit }) {
  ...
  return (
    <div className="card p-4 bg-zinc-950 border border-zinc-800 rounded-xl">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-xs font-bold text-zinc-100">{slot.label}</div>
            <div className="text-[10px] text-zinc-500">{slot.time_range}</div>
          </div>
        </div>
        <button
          type="button"
          onClick={() => onEdit(slot)}
          className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-100 bg-zinc-900 border border-zinc-800"
        >
          <Pencil className="w-3 h-3" />
        </button>
      </div>

      <div className="w-full h-1.5 bg-zinc-900 rounded-full overflow-hidden my-2.5">
        <div
          className="h-full rounded-full bg-emerald-500 transition-all duration-500"
          style={{ width: `${usedPct}%` }}
        />
      ...
      <div className="flex items-center justify-between text-[11px] text-zinc-400">
        <span>
          Terisi: <strong className="text-zinc-200">{slot.booked} pax</strong>
        </span>
        <span>
          Kapasitas: <strong className="text-zinc-200">{slot.max_capacity} pax</strong>
        </span>
      </div>
    </div>
  );
...
function EditQuotaModal({ day, onClose, onSave }) {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content card p-6 max-w-sm w-full bg-zinc-900 border border-zinc-800 rounded-2xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-bold text-zinc-100">Ubah Kuota {day.dayLabel}</h3>
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full btn-primary py-2.5 justify-center text-xs font-semibold"
          >
...
function EditTimeSlotModal({ slot, onClose, onSave }) {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content card p-6 max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-bold text-zinc-100">Ubah Sesi Kunjungan</h3>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-200">
            <X className="w-4 h-4" />
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-zinc-800">
        <div>
          <div className="text-xs text-emerald-500 uppercase tracking-widest font-semibold flex items-center gap-2 mb-1">
            ...
            <span>Passify Quota & Capacity Control • Tremor UI Powered</span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <AdminStatCard
          icon={ShieldCheck}
          label="Status Daya Dukung Alam"
          value="Aman (62%)"
          subValue="Sesuai rekomendasi konservasi TN"
          badgeText="ECOLOGY"
        />
      </div>
      ...
      <div className="card p-5 bg-zinc-950 border border-zinc-800 rounded-xl shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-zinc-500">Klik ikon pensil untuk ubah kuota</span>
        </div>
