  ...
  return (
    <section id="catalog-section" className="mx-auto max-w-[1240px] px-4 pb-16 pt-6 sm:px-6 sm:pb-20 lg:px-8">
      <div className="flex flex-col gap-4 border-b border-[var(--border)] pb-5 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm font-medium text-[var(--ink-soft)]">{filteredDestinations.length} tujuan tersedia untuk dijelajahi</p>
      ...
      {filteredDestinations.length === 0 ? (
        <div className="border-b border-[var(--border)] py-14 text-center">
          <span className="mx-auto grid h-10 w-10 place-items-center rounded-full bg-[var(--leaf-pale)] text-[var(--forest)]"><Layers className="h-4 w-4" /></span>
                ...
                >
                  <div className="relative h-48 overflow-hidden bg-[var(--leaf-pale)]">
                    ...
                    <div className="absolute bottom-3 left-3 flex items-center gap-2 text-[11px] font-bold">
                      <span className="rounded-full bg-white/90 px-2.5 py-1.5 text-[var(--forest)] backdrop-blur-sm">{destination.typeLabel}</span>
                      <span className="inline-flex items-center gap-1 rounded-full bg-[var(--bark-pale)] px-2.5 py-1.5 text-[var(--bark)] backdrop-blur-sm"><Star className="h-3 w-3 fill-current" />{destination.rating}</span>
                    </div>
                  </div>

                  <div className="p-5">
                    <div className="flex items-center gap-1.5 text-xs font-semibold text-[var(--ink-soft)]"><MapPin className="h-3.5 w-3.5 text-[var(--forest-soft)]" />{destination.province}</div>
                    <h3 className="mt-2 font-serif text-2xl font-semibold leading-tight tracking-[-0.035em] text-[var(--forest-deep)] group-hover:text-[var(--forest)]">{destination.name}</h3>
                    <p className="mt-2 line-clamp-2 text-xs leading-5 text-[var(--ink-soft)]">{destination.description}</p>
