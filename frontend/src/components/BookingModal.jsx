        ...
        <form onSubmit={handleSubmitBooking} className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div>
                <span className="text-[11px] text-slate-400 block mb-1">Tanggal Kunjungan</span>
                <input
                  ...
                  className="w-full bg-slate-900 border border-slate-700/80 rounded-xl px-3 py-2 text-sm text-slate-200 focus:border-emerald-500 focus:outline-none"
                  required
                />
              </div>

              <div>
                <span className="text-[11px] text-slate-400 block mb-1">Sesi Kunjungan (Time Slot)</span>
                <select
              ...
              {destination.ticket_categories.map((cat) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Qty Controls */}
                    <div className="flex items-center gap-2 bg-slate-800 p-1 rounded-lg border border-slate-700/60">
                      <button
                        type="button"
                        id={`btn-dec-${cat.id}`}
                        onClick={() => handleQtyChange(cat.id, -1)}
                        className="w-7 h-7 rounded bg-slate-700 text-slate-200 font-bold hover:bg-slate-600 flex items-center justify-center"
                      >
                        -
                      </button>
                      ...
                      <button
                        ...
                        id={`btn-inc-${cat.id}`}
                        onClick={() => handleQtyChange(cat.id, 1)}
                        className="w-7 h-7 rounded bg-emerald-600 text-white font-bold hover:bg-emerald-500 flex items-center justify-center"
                      >
                        +
                      </button>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <input
                  ...
                  className="w-full bg-slate-900 border border-slate-700/80 rounded-xl px-3 py-2 text-sm text-slate-200 placeholder-slate-500 focus:border-emerald-500 focus:outline-none"
                  required
                />
              </div>
              <div>
          ...
          <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-2 text-xs">
            <div className="flex justify-between text-slate-400">
              ...
              <span className="text-slate-200 font-semibold">Rp {totalTicketPrice.toLocaleString('id-ID')}</span>
            </div>
            <div className="flex justify-between text-slate-400">
              <span>Asuransi Jiwa & Retribusi Pemda</span>
              <span className="text-slate-200 font-semibold">Rp {(totalInsurance + totalRetribusi).toLocaleString('id-ID')}</span>
            </div>
            <div className="flex justify-between text-slate-400">
              <span>Biaya Layanan SaaS Passify</span>
              <span className="text-slate-200 font-semibold">Rp {platformFee.toLocaleString('id-ID')}</span>
            </div>
            <div className="pt-2 border-t border-slate-800 flex justify-between items-center text-sm">
              ...
              <span className="font-extrabold text-emerald-400 text-lg font-['Outfit']">
          ...
          <button
            ...
            className="w-full btn-primary py-3.5 justify-center text-base font-bold shadow-lg"
          >
            {isSubmitting ? (
              <span className="flex items-center gap-2">
                <span className="animate-spin">⏳</span> Memproses Pemesanan...
              </span>
            ) : (
              <span className="flex items-center gap-2">
