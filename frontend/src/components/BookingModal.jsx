      ...
      <div>
        <div className="border-b border-[var(--border)] bg-[var(--forest-deep)] px-5 pb-5 pt-6 text-white sm:px-7 sm:pt-7 relative">
          <button
            ...
            type="button"
            onClick={onClose}
            className="absolute right-5 top-5 grid min-h-[44px] min-w-[44px] place-items-center rounded-xl border border-white/20 bg-white/10 text-white transition-all hover:bg-white/20 active:scale-95 cursor-pointer"
          ...
          <h2 className="mt-2 max-w-md pr-12 font-serif text-2xl font-bold text-white sm:text-3xl">{destination.name}</h2>
        ...
        <form onSubmit={handleSubmit}>
          <div className="min-h-[340px] space-y-5 p-5 sm:p-7">
            ...
            {activeStep === 3 && <section aria-labelledby="booking-step-title">
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2"><div className="surface-muted p-4 rounded-xl border border-[var(--border)]"><span className="text-[10px] font-extrabold uppercase tracking-wide text-[var(--ink-muted)]">Jadwal kunjungan</span><strong className="mt-2 block text-sm text-[var(--forest-deep)]">{new Intl.DateTimeFormat('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(`${visitDate}T00:00:00`))}</strong><span className="mt-1 block text-xs text-[var(--ink-soft)]">{selectedSlot?.label || 'Sesi belum dipilih'}</span></div><div className="surface-muted p-4 rounded-xl border border-[var(--border)]"><span className="text-[10px] font-extrabold uppercase tracking-wide text-[var(--ink-muted)]">Pengunjung</span><strong className="mt-2 block text-sm text-[var(--forest-deep)]">{totals.quantity} tiket</strong><span className="mt-1 block truncate text-xs text-[var(--ink-soft)]">{visitors.map((visitor) => visitor.name).join(', ')}</span></div></div>
              <div className="mt-5 rounded-2xl border border-emerald-900/15 bg-white/75 dark:bg-black/40 backdrop-blur-md p-5 shadow-xs"><div className="space-y-2 text-xs text-[var(--ink-soft)]"><div className="flex justify-between gap-4"><span>Subtotal tiket ({totals.quantity})</span><strong className="text-[var(--ink)]">{formatRupiah(totals.ticket)}</strong></div><div className="flex justify-between gap-4"><span>Asuransi & retribusi</span><strong className="text-[var(--ink)]">{formatRupiah(totals.insurance + totals.retribution)}</strong></div><div className="flex justify-between gap-4"><span>Biaya sistem</span><strong className="text-[var(--ink)]">{formatRupiah(platformFee)}</strong></div></div><div className="mt-4 flex items-end justify-between border-t border-[var(--forest-soft)]/30 pt-4"><span className="font-bold text-[var(--forest-deep)]">Total pembayaran</span><strong className="font-serif text-2xl text-[var(--bark)]">{formatRupiah(grandTotal)}</strong></div></div>
            </section>}
          ...
          <div className="sticky bottom-0 flex items-center justify-between gap-3 border-t border-[var(--border)] bg-[rgba(255,254,250,.96)] px-5 py-4 backdrop-blur-sm sm:px-7">
            {activeStep > 1 ? <button type="button" onClick={() => { setValidationMessage(''); setActiveStep((step) => step - 1); }} className="btn-secondary min-h-[44px] px-4"><ArrowLeft className="h-4 w-4" />Kembali</button> : <span className="text-xs font-semibold text-[var(--ink-soft)]">Aman &amp; transparan</span>}
            {activeStep < 3 ? <button type="submit" className="btn-primary min-h-[44px] px-6">Lanjutkan<ArrowRight className="h-4 w-4" /></button> : <button id="submit-booking-btn" type="submit" disabled={isSubmitting} className="btn-clay min-h-[44px] px-6 disabled:cursor-not-allowed disabled:opacity-60">{isSubmitting ? 'Memproses pesanan…' : 'Konfirmasi & bayar'}<ArrowRight className="h-4 w-4" /></button>}
          </div>
        </form>
      </div>
    </ModalWrapper>
