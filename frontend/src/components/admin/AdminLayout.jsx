    ...
    <div className="admin-shell min-h-screen">
      ...
      <div className="min-h-screen transition-[margin] duration-200" style={{ marginLeft: collapsed ? 76 : 264 }}>
        <header className="sticky top-0 z-30 flex min-h-[68px] items-center justify-between gap-3 border-b border-[var(--border)] bg-[rgba(255,254,250,.9)] px-4 py-3 backdrop-blur-md sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-3"><button type="button" onClick={() => setMobileOpen(true)} className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-[var(--border)] text-[var(--forest)] hover:bg-[var(--leaf-pale)] md:hidden" aria-label="Buka menu"><Menu className="h-4 w-4" /></button><div className="min-w-0"><p className="hidden text-[10px] font-extrabold uppercase tracking-[.14em] text-[var(--forest-soft)] sm:block">Mode pengelola · {TENANT_INFO.name}</p><h1 className="truncate text-lg font-bold text-[var(--forest-deep)] sm:mt-0.5 sm:text-xl">{currentItem?.label || 'Ringkasan'}</h1></div></div>
