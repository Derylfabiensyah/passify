      ...
      <main className="mx-auto max-w-[1240px] px-4 pb-28 pt-5 sm:px-6 sm:pt-7 lg:px-8">
        <section className="relative isolate overflow-hidden rounded-[2rem] bg-[var(--forest-deep)] shadow-[var(--shadow-lift)]">
          ...
          <div className="grid gap-8 px-6 py-9 sm:px-9 sm:py-11 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-end lg:px-12 lg:py-14">
            ...
            <div className="rounded-[1.25rem] border border-white/15 bg-[rgba(255,254,250,.96)] p-5 shadow-[0_16px_36px_rgba(0,0,0,.15)]"><p className="eyebrow">Ketersediaan hari ini</p><div className="mt-4 flex items-end justify-between"><span className="text-xs font-semibold text-[var(--ink-soft)]">Kuota terisi</span><strong className="font-serif text-2xl">{used}%</strong></div><div className="mt-2 h-2.5 overflow-hidden rounded-full bg-[var(--leaf-pale)]"><div className="h-full rounded-full bg-[var(--leaf)]" style={{ width: `${used}%` }} /></div><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl border border-[var(--border)] bg-[var(--canvas)] p-3"><span className="block text-[10px] font-bold uppercase tracking-wide text-[var(--ink-muted)]">Tersisa</span><strong className="mt-1 block font-serif text-xl">{remaining.toLocaleString('id-ID')}</strong></div><div className="rounded-xl border border-[var(--border)] bg-[var(--canvas)] p-3"><span className="block text-[10px] font-bold uppercase tracking-wide text-[var(--ink-muted)]">Mulai dari</span><strong className="mt-1 block font-serif text-lg text-[var(--bark)]">{rupiah(startingPrice)}</strong></div></div></div>
          </div>
        </section>

        <section className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_280px]"><div><p className="eyebrow">Pilih tiket</p><h2 className="mt-2 text-3xl font-bold">Satu perjalanan, satu tiket resmi</h2><p className="mt-2 text-sm text-[var(--ink-soft)]">Pilih tiket yang sesuai, kemudian lengkapi jadwal dan data pengunjung dalam tiga langkah singkat.</p><div className="mt-6 grid gap-4 md:grid-cols-2">{(destination.ticket_categories || []).map((category) => { const total = Number(category.price || 0) + Number(category.insurance || 0) + Number(category.retribusi || 0); return <article key={category.id} className="card flex min-h-[200px] flex-col justify-between p-5 hover:-translate-y-0.5 hover:shadow-[var(--shadow-soft)]"><div><span className="inline-flex rounded-full bg-[var(--leaf-pale)] px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wide text-[var(--forest)]">E-ticket resmi</span><h3 className="mt-4 text-xl font-bold">{category.name}</h3><p className="mt-2 text-xs text-[var(--ink-soft)]">Termasuk komponen resmi sesuai kebijakan kawasan.</p></div><div className="mt-5 flex items-end justify-between border-t border-[var(--border)] pt-4"><div><span className="block text-[10px] font-bold uppercase tracking-wide text-[var(--ink-muted)]">Total per orang</span><strong className="mt-1 block font-serif text-xl text-[var(--bark)]">{rupiah(total)}</strong></div><button type="button" onClick={openBooking} className="btn-primary btn-sm">Pilih<ArrowRight className="h-3.5 w-3.5" /></button></div></article>; })}</div></div>
          <aside className="surface-muted h-fit p-5 lg:sticky lg:top-24"><CalendarDays className="h-5 w-5 text-[var(--forest)]" /><h2 className="mt-4 text-xl font-bold">Kunjungan tertata</h2><ul className="mt-4 space-y-3 text-xs leading-5 text-[var(--ink-soft)]"><li className="flex gap-2"><CheckCircle2 className="h-4 w-4 shrink-0 text-[var(--forest)]" />Pilih tanggal dan sesi yang sesuai.</li><li className="flex gap-2"><CheckCircle2 className="h-4 w-4 shrink-0 text-[var(--forest)]" />Isi data setiap pemegang tiket.</li><li className="flex gap-2"><CheckCircle2 className="h-4 w-4 shrink-0 text-[var(--forest)]" />Tunjukkan QR aktif di gerbang.</li></ul></aside>
        </section>

        <section className="mt-10 grid gap-5 md:grid-cols-2"><article className="card p-6"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--leaf-pale)] text-[var(--forest)]"><Leaf className="h-5 w-5" /></span><h2 className="text-xl font-bold">Fasilitas kawasan</h2></div><ul className="mt-5 grid gap-2 sm:grid-cols-2">{(destination.facilities || []).map((facility) => <li key={facility} className="flex items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--canvas)] px-3 py-2.5 text-xs font-semibold text-[var(--ink-soft)]"><CheckCircle2 className="h-4 w-4 shrink-0 text-[var(--forest)]" />{facility}</li>)}</ul></article><article className="rounded-[var(--radius)] border border-[var(--bark)]/25 bg-[var(--bark-pale)]/55 p-6"><p className="eyebrow !text-[var(--bark)]">Etika berkunjung</p><h2 className="mt-2 text-xl font-bold">Jaga kawasan bersama</h2><p className="mt-4 text-sm leading-6 text-[var(--ink-soft)]">{destination.rules}</p></article></section>
      </main>
